...
export function getTruckingJobResult(from: any, to: any, customerId: any) {
    ...
    where tdt.trucking_type = 'commercial' AND tdt.is_deleted = false ${whereClause}
    ...
    where tdt.trucking_type = 'commercial' AND tdt.customer_id = '${customerId}' AND tdt.is_deleted = false ${whereClause}
    ...
    WHERE tdt.trucking_type = 'commercial' AND tdt.is_deleted = false
    GROUP BY tr.rate, tr.rate_type, tr.rate;
    ...
    where tdt.customer_id = '${customerId}' AND  trucking_type = 'commercial' AND tdt.is_deleted = false; ${whereClause}
    `
    return query;
}